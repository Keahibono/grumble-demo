...
TrafficController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('trafficFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
WorkController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('workFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
SchoolController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('schoolFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
PeopleController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('peopleFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
WeatherController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('weatherFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
PoliticsController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('politicsFeed', function() {
      dataReadyHold.release();
    });
  }
});

TechController = RouteController.extend({
  onBeforeAction: function() {
    Meteor.subscribe('techFeed', function() {
      dataReadyHold.release();
    });
  }
});
...
Router.map(function() {
  ...
  this.route('feed');
  this.route('trafficFeed');
  this.route('workFeed');
  this.route('schoolFeed');
  this.route('peopleFeed');
  this.route('weatherFeed');
  this.route('politicsFeed');
  this.route('techFeed');
  this.route('recipes');
