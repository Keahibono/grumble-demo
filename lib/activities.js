...
Activities.traffic = function() {
  return Activities.find({ text: { $in: [/traffic/] }, sort: {date: -1} } );
}

Activities.work = function() {
  return Activities.find({ text: { $in: [/work/, /boss/] }, sort: {date: -1} } );
}

Activities.school = function() {
  return Activities.find({ text: { $in: [/school/, /homework/, /study/, /finals/, /teacher/, /exam/, /college/, /university/, /universities/, /test/] }, sort: {date: -1} } );
}

Activities.people = function() {
  return Activities.find({ text: { $in: [/people/, /friend/, /family/, /sister/, /brother/, /mom/, /dad/, /coworker/, /dude/, /bitch/, /jerk/, /asshole/, /bf/, /gf/] }, sort: {date: -1} } );
}

Activities.weather = function() {
  return Activities.find({ text: { $in: [/weather/, /muggy/, /humid/, /chilly/, /hurricane/, /rain/, /heat/] }, sort: {date: -1} } );
}
